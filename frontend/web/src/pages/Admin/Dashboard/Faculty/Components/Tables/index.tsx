...
interface Props {
    checked:            boolean
    state:              IFaculty[]
    departments:        IFaculty[]
    setState:           React.Dispatch<React.SetStateAction<IFaculty[]>>
    checkbox:           React.MutableRefObject<HTMLInputElement | null>
    toggleAll:          () => void
}

const FacultyTable: React.FC<Props> = ({state, setState, departments, checkbox, checked, toggleAll}) => {
                                ...
                                {departments.map((department) => (
                                    <tr key={department.id} className={state.includes(department) ? 'bg-gray-50' : undefined}>
                                        ...
                                        <td className="whitespace-pre-wrap px-3 py-4 text-sm space-x-1 space-y-1 text-gray-500">
                                            {department.Subject.length && department.Subject.map((sub) => 
                                                <Badges text={sub} />)
                                            }
                                        </td>
                                        <td className="whitespace-pre-wrap px-3 py-4 text-sm space-x-1 space-y-1 text-gray-500">
                                        ...
                                        <td className="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <Link to="/dashboard/department" className="text-blue-600 hover:text-blue-900">
                                                Edit<span className="sr-only">, {department.firstName + ' ' + department.lastName}</span>
                                            </Link>
                                        </td>
                                    </tr>
                                ))}
